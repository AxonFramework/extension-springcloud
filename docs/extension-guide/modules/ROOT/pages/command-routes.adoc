...
- `RoutingStrategy` - The component in charge of deciding which of the nodes receives the commands consistently. By default, a `AnnotationRoutingStrategy` is used (see xref:axon_framework_ref:axon-framework-commands:infrastructure.adoc#DistributedCommandBus[Distributing the Command Bus] for more).
